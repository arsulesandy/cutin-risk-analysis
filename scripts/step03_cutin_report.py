...
def _write_per_recording_csv(rows: list[dict[str, int | float | str]], out_path: Path) -> None:
    ...
    with out_path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["recording_id", "vehicles", "lane_changes", "cutins", "cutin_ratio"],
        )
        writer.writeheader()
    ...
    if sorted_rows:
        body_rows = [
            [
                ...
                row["lane_changes"],
                row["cutins"],
                f"{float(row['cutin_ratio']):.2%}",
            ]
            for row in sorted_rows
        ]
    else:
        body_rows = [["n/a", "n/a", "n/a", "n/a", "n/a"]]

    return markdown_table(
        headers=["Recording", "Vehicles", "Lane changes", "Cut-ins", "Cut-in ratio"],
        rows=body_rows,
...
def _build_details_markdown(
    *,
    ...
    total_cutins: int,
    overall_lane_change_duration_frames: list[float],
    overall_lane_change_duration_seconds: list[float],
    overall_relation_duration_frames: list[float],
    overall_relation_duration_seconds: list[float],
    timestamp: str,
) -> str:
    lines = [
        f"# {step_display_name(STEP_NUMBER)} Cut-in Detailed Statistics",
        "",
        f"- Generated at: {timestamp}",
        f"- Dataset root: `{dataset_root}`",
        "",
        "## Detection configuration",
        f"- Lane-change `min_stable_before_frames`: {lane_options.min_stable_before_frames}",
        f"- Lane-change `min_stable_after_frames`: {lane_options.min_stable_after_frames}",
        f"- Cut-in `search_window_frames`: {cutin_options.search_window_frames}",
        f"- Cut-in `min_relation_frames`: {cutin_options.min_relation_frames}",
        f"- Cut-in `max_relation_delay_frames`: {cutin_options.max_relation_delay_frames}",
    ]
    ...
    if total_lane_changes:
        lines.append(f"- Overall cut-in ratio: {total_cutins / total_lane_changes:.2%}")

    lines.extend(["", "### Top cut-in transitions (dataset-wide)"])
    ...
    lines.append(f"- Events included: {len(overall_lane_change_duration_frames)} lane changes")
    lines.extend(
        _summary_lines(
            "Duration (frames)",
            _numeric_summary(overall_lane_change_duration_frames),
            digits=0,
            include_count=False,
        )
    )
    lines.extend(
        _summary_lines(
            "Duration (seconds)",
            _numeric_summary(overall_lane_change_duration_seconds),
            digits=3,
            include_count=False,
        )
    )
    lines.extend(["", "### Cut-in relation duration stats (dataset-wide)"])
    lines.append(f"- Events included: {len(overall_relation_duration_frames)} cut-ins")
    lines.extend(
        _summary_lines(
            "Duration (frames)",
            _numeric_summary(overall_relation_duration_frames),
            digits=0,
            include_count=False,
        )
    )
    lines.extend(
        _summary_lines(
            "Duration (seconds)",
            _numeric_summary(overall_relation_duration_seconds),
            digits=3,
            include_count=False,
        )
    )
...
def main() -> None:
    ...
    print(f"{step_display_name(STEP_NUMBER)}: Cut-in Report")
    ...
    for _, _, rid in iter_with_progress(
        ...
        try:
            ...
            overall_transition_counts.update((e.from_lane, e.to_lane) for e in all_cutins)

            lane_change_duration_frames = [float(e.end_frame - e.start_frame + 1) for e in all_lane_changes]
            lane_change_duration_seconds = [float(max(0.0, e.end_time - e.start_time)) for e in all_lane_changes]
            relation_duration_frames = [float(e.relation_end_frame - e.relation_start_frame + 1) for e in all_cutins]
            relation_duration_seconds = [float(max(0.0, e.relation_end_time - e.relation_start_time)) for e in all_cutins]

            overall_lane_change_duration_frames.extend(lane_change_duration_frames)
            overall_lane_change_duration_seconds.extend(lane_change_duration_seconds)
            overall_relation_duration_frames.extend(relation_duration_frames)
            overall_relation_duration_seconds.extend(relation_duration_seconds)

            per_recording_rows.append(
                {
                    "recording_id": rid,
                    "vehicles": int(df_all["id"].nunique()),
                    "lane_changes": len(all_lane_changes),
                    "cutins": len(all_cutins),
                    "cutin_ratio": (len(all_cutins) / len(all_lane_changes)) if all_lane_changes else 0.0,
                }
            )
    ...
    summary_csv_path = report_dir / "cutin_counts_by_recording.csv"
    _write_per_recording_csv(per_recording_rows, summary_csv_path)

    details_md_path = report_dir / "cutin_details.md"
    details_md_path.write_text(
        _build_details_markdown(
            dataset_root=root,
            lane_options=lane_change_options,
            ...
            overall_lane_change_duration_frames=overall_lane_change_duration_frames,
            overall_lane_change_duration_seconds=overall_lane_change_duration_seconds,
            overall_relation_duration_frames=overall_relation_duration_frames,
            overall_relation_duration_seconds=overall_relation_duration_seconds,
