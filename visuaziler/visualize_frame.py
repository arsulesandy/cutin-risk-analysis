...
class VisualizationPlot(object):
        ...
        for track in self.tracks:
            ...
            if initial_frame <= self.current_frame < final_frame:
                ...
                if self.arguments["plotBoundingBoxes"]:
                    rect = plt.Rectangle((bounding_box[0], vehicle_box_y), bounding_box[2],
                ...
                if self.arguments["plotTextAnnotation"]:
                    ...
                    if show_id_on_vehicle:
                        vehicle_text_location = (bounding_box[0] + (bounding_box[2] / 2),
                                                 vehicle_box_y + (bounding_box[3] / 2))
                        id_text_patch = self.ax.annotate("{}".format(track_id), xy=vehicle_text_location,
                                                         xytext=vehicle_text_location, **id_on_vehicle_text_style)
    ...
    def plot_highway_information(self):
        info_text = "Direction 1, X-Velocity is Negative, Sign = -1, Lane 1, 2"
        info_text_bottom = "Direction 2, X-Velocity is Positive, Sign = 1, Lane 1, 2"
        info_box_style = dict(boxstyle="round,pad=0.2", fc="white", alpha=.7, ec="black", lw=0.2)
        self.ax.text(0.5, 0.99, info_text, transform=self.ax.transAxes, ha="center", va="top", fontsize=10,
                     color="black", bbox=info_box_style, zorder=30)
        self.ax.text(0.5, 0.01, info_text_bottom, transform=self.ax.transAxes, ha="center", va="bottom", fontsize=10,
                     color="black", bbox=info_box_style, zorder=30)
