...
% --- Abstract ---
    ...
    When a vehicle cuts into another driver's lane on a highway, the follower may need to brake or adjust speed to maintain a safe gap. Quantifying these interactions at scale is important for traffic safety assessment and for building scenario libraries that support automated driving validation. However, most analysis pipelines depend on dataset-provided lane and neighbour identifiers, which limits reuse when these fields are unavailable or inconsistently defined across datasets. This thesis presents \FrameworkName{}, a minimal-input framework that reconstructs lane assignment and same-lane predecessor/follower relations from trajectory geometry ($x,y$) and lane-marking metadata.
