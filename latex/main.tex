...
% ==============================================================================
  ...
  \item \textbf{Recovery:} $[t_0+\SI{2}{\second},\, t_0+\SI{4}{\second}]$
    ...
    Figure~\ref{fig:stage-timeline} illustrates the four-stage alignment around $t_0$.
  ...
  \item rows: \{ahead of the cutter, alongside the cutter, behind the cutter\}.
    ...
    \begin{figure}[H]
        ...
        \caption{Construction of an interaction context signature. For each frame, the local traffic scene around the cutter is mapped to a 3$\times$3 semantic occupancy grid, mirror-normalised to a canonical direction, embedded into a fixed 4$\times$4 grid, and encoded as a reversible 16-bit Hilbert-order code.}
...
% ==============================================================================
    ...
    \FrameworkName{} takes trajectories, vehicle geometry, and lane-marking metadata as inputs. It then reconstructs lane assignments, neighbour relations, cut-in events, pair-specific safety indicators, and context signatures. To evaluate each processing step against the dataset labels, the framework is run in two parallel paths:
  ...
  \item \textbf{Reconstruction path:} uses only trajectories and lane markings to produce the same outputs without using highD's derived relational fields as inputs.
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[
            font=\small,
            block/.style={draw, rounded corners=2pt, line width=0.9pt, minimum height=11mm, text width=3.7cm, align=center, inner sep=5pt},
            inputblock/.style={block, text width=3.4cm},
            line/.style={-{Latex[length=3.2mm,width=2.1mm]}, line width=0.95pt},
            group/.style={draw, rounded corners=4pt, dashed, line width=0.8pt, inner sep=6pt}
        ]
...
\bottomrule
  ...
  \item \texttt{XX\_recordingMeta.csv} (recording metadata, including lane markings).
    ...
    The dashboard was used to inspect representative cases and extremes (e.g., lowest-TTC events) and to confirm that reconstructed relations and occupancy signatures correspond to the visible scene. It served as a support tool for validation and communication rather than as a standalone thesis contribution: it was not used to manually correct events or labels, and all reported results are produced by the framework's deterministic components. Figure~\ref{fig:gui-dashboard} shows the interface used during this qualitative inspection.
