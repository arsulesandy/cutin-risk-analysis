...
% --- Abstract ---
    ...
    Lane identifiers and neighbour identifiers are important when studying vehicle interactions in trajectory datasets. A common limitation, however, is that many analysis pipelines depend on these derived fields directly, which reduces reuse when the fields are unavailable, inconsistent, or defined differently across datasets. This thesis presents \FrameworkName{}, a minimal-input framework that reconstructs lane assignment and same-lane predecessor/follower relations from trajectory geometry ($x,y$) and lane-marking metadata.

    Based on these reconstructed relations, the framework detects cut-ins as explicit cutter--follower pairs, computes distance headway (DHW), time headway (THW), time-to-collision (TTC), and deceleration rate to avoid crash (DRAC) for that pair, and represents the surrounding traffic context with an ego-centric occupancy grid. The grid is mirror-normalised and encoded as a reversible 16-bit Hilbert space-filling-curve signature to make the context comparable across travel directions.

    The framework is evaluated on the \textit{highD} dataset, where highD lane and neighbour identifiers are used only as reference labels and not as method inputs. Across recordings 01--60, same-lane predecessor/follower reconstruction with inferred lanes reaches mean accuracy 0.999810/0.999811, and cut-in events detected from reconstructed relations match the reference-label events with $F_1 = 0.998938$. The detector extracts 10,026 cut-ins, of which 10,023 remain after stage-window validity checks. For 1,421,980 stage rows, reconstructed and reference context signatures agree exactly in 99.49\% of cases. The SFC encoding is verified through exhaustive round-trip testing over all $2^{16}$ code values. The mirrored decision-stage signatures yield seven stable context archetypes; the most risk-enriched archetype shows a 1.32$\times$ higher execution-stage THW risk prevalence than the overall event set.

    Beyond reconstruction accuracy, a leave-one-recording-out cross-validation experiment shows that decision-stage features predict execution-stage THW risk with ROC-AUC 0.81. SFC context features alone achieve AUC 0.61, showing that the spatial context signatures carry independent risk-relevant information beyond scalar indicators. The addition of DRAC as a deceleration-based indicator shows that the framework's pair-consistent computation extends naturally to complementary safety measures. Most detected cut-ins (99.9\%) involve routine gap acceptance rather than hard-braking situations.
...
% ==============================================================================
  ...
  \item \textbf{Reversible context signatures:} a direction-normalised 3$\times$3 occupancy grid encoded into a 16-bit Hilbert SFC signature, verified against highD reference occupancy (99.49\% exact row agreement over 1.4\,M stage rows).
  \item \textbf{Predictive evaluation:} a LORO-CV experiment showing that decision-stage features predict execution-stage THW risk with ROC-AUC 0.81; SFC context features alone achieve AUC 0.61, showing that the framework produces predictive features beyond descriptive agreement.
  \item \textbf{Reproducible two-path evaluation:} the minimal-input reconstruction path and the reference-label path are executed in parallel, producing deterministic outputs that can be regenerated from the same settings.
    ...
    Chapter~\ref{chap:relatedwork} reviews the dataset, related work, and research gap. Chapter~\ref{chap:researchmethod} describes the evaluation methodology. Chapter~\ref{chap:theory} formally defines cut-in events, stage models, safety indicators, and SFC signatures. Chapter~\ref{chap:methods} details the framework components and the predictive evaluation protocol. Chapter~\ref{chap:results} reports results. Chapter~\ref{chap:discussion} discusses implications and limitations, and Chapter~\ref{chap:conclusion} concludes.
...
% ==============================================================================
    ...
    Because crashes are rare in naturalistic data, surrogate safety measures (SSMs) are widely used to assess interaction severity without requiring crash observations. The main indicators used in this thesis are time-to-collision (TTC), time headway (THW), distance headway (DHW), and the deceleration rate to avoid crash (DRAC). Recent reviews emphasise that SSM values depend strongly on modelling assumptions, measurement noise, and traffic state, and therefore require careful interpretation~\cite{wang2021ssm,johnsson2021validation}. Archer~\cite{archer2005indicators} introduced DRAC as a deceleration-based complement to time-based indicators, with values above \SI{3.35}{\meter\per\second\squared} commonly interpreted as hard-braking situations.
    ...
    Scalar indicators (e.g., minimum THW) compress interactions to a few values and may miss relevant local traffic structure. Grid-based representations such as occupancy grids are widely used for compactly describing local surroundings \cite{elfes1989}.

    To compress spatial grids into a compact signature while preserving locality, Hilbert-type space-filling curves order 2D cells into 1D sequences \cite{moon2001}. This thesis uses a small ego-centric 3$\times$3 occupancy grid encoded into a 16-bit SFC signature for a reversible integer-code representation of local context.
...
% ==============================================================================
    ...
    Generative AI tools were used in a supportive role for troubleshooting development environment issues, resolving runtime errors, and suggesting code refactoring patterns. All research ideas, problem formulations, methodological decisions, evaluation design, and interpretation of results were developed independently by the authors. The authors take full responsibility for the correctness and integrity of the work.
  ...
  \item \textbf{Recovery:} $[t_0+\SI{2}{\second},\, t_0+\SI{4}{\second}]$
    ...
    The choice of \SI{2}{\second} per stage (and \SI{4}{\second} total pre-onset window) is grounded in empirical findings from the lane-change literature. Wang et al.~\cite{wang2019cutin} report that cut-in durations in naturalistic data range from 0.7 to \SI{12.4}{\second} with a mean of approximately \SI{3.9}{\second}, while Yang et al.~\cite{yang2019lanechange} find that lane-change durations and gap acceptance windows are typically concentrated in the 2 to \SI{6}{\second} range. Zhou and Itoh~\cite{zhou2016riskperception} identify two distinct phases of risk perception during lane changes: an initial phase where drivers assess the situation primarily through TTC, and a later phase during lane insertion where both TTC and THW become relevant. Splitting the pre-onset window at \SI{2}{\second} separates these two phases. On the post-onset side, a \SI{2}{\second} execution window captures the initial gap closure after the cutter--follower relation is established, and a \SI{2}{\second} recovery window covers the subsequent stabilisation period. The $\pm\SI{4}{\second}$ analysis window is also consistent with intention recognition studies that use multi-second observation windows preceding the lane-change manoeuvre for reliable prediction~\cite{liu2021intention}. Overall, the \SI{2}{\second} split provides a balanced partition that aligns with the empirical phase structure reported in the literature.
  ...
  \]
    ...
    \[
        ...
        \end{cases}
    \]
    DRAC represents the minimum constant deceleration the follower must apply to avoid a collision under constant-speed assumptions. Values above \SI{3.35}{\meter\per\second\squared} are commonly interpreted as a hard-braking situation \cite{archer2005indicators,wang2021ssm}. Unlike TTC, which describes \emph{time until} a potential collision, DRAC describes the \emph{deceleration required} to prevent one, providing a complementary perspective on interaction severity. DRAC is only defined when the follower is closing, the same condition as TTC. We report DRAC alongside TTC and THW in the execution stage to see how often closing events require non-trivial braking demand.
  ...
  \item rows: \{preceding, alongside, following\}.
    ...
    We perform an exhaustive round-trip test across all $2^{16}$ possible bit patterns to verify the encoding; encoding followed by decoding returns the original grid for both Hilbert and Morton (Z-order) traversals.
...
% ==============================================================================
  ...
  \item \textbf{Reconstruction path:} uses only $x,y$ trajectories and lane markings to produce the same outputs without accessing highD's derived fields.
    ...
    \begin{figure}[H]
        \centering
        \safeincludegraphics[width=0.98\linewidth]{fig_framework_overview_academic.pdf}
...
\bottomrule
    ...
    The remainder of this chapter describes each component. Qualitative verification was additionally performed using an interactive inspection dashboard (Appendix~\ref{app:dashboard}).

    \section{Configuration}
    Table~\ref{tab:config} summarises the key parameters used in the final run (highD 01--60, \SI{25}{\hertz}).
...
\bottomrule
    ...
    These parameters are informed by empirical cut-in duration distributions~\cite{wang2019cutin} and the two-phase risk perception model~\cite{zhou2016riskperception}, and are reported to support reproducibility.
...
\Return no event\;
    ...
    The framework separates event extraction from risk labelling. Lane changes are detected between stable segments (reducing sensitivity to label jitter), and follower pairing requires a minimum persistence duration to avoid pairing transient overtake encounters. DHW, THW, and TTC are computed for the explicit cutter--follower pair rather than a generic predecessor, consistent with recent risk models that use interaction-specific features~\cite{shangguan2022,liu2025multitask}. Following Zhou and Itoh~\cite{zhou2016riskperception}, we interpret TTC and THW as complementary indicators across the stage-aligned window.
    ...
    As an additional analysis of representational usefulness, we cluster event-level decision-stage context signatures after the core verification step. For each validated cut-in event, the mirror-normalised decision-stage 3$\times$3 occupancy grids are averaged over the frames in that stage, yielding a 9-dimensional vector of cell-wise occupancy probabilities. We then apply $k$-means clustering for $K=3,\dots,8$, assess internal cohesion with the silhouette score, and assess solution stability with the mean adjusted Rand index (ARI) across repeated random initialisations. This clustering is not used as a primary correctness criterion; it is reported only as an exploratory check of whether the verified SFC representation supports interpretable grouping of decision-stage interaction contexts.
...
\ForEach{frame $t$ in the analysis window around $t_0$}{
    ...
    \paragraph{Model.}
    We train a bagged decision-stump ensemble (50 trees with random feature subsampling), which provides both predictions and feature-importance estimates via Gini gain.
...
\bottomrule
    ...
    \begin{table}[H]
        ...
        \midrule
        Mean ROC-AUC       & 0.810 \\
        Mean $F_1$ (risky class) & 0.518 \\
        Mean precision     & 0.376 \\
        Mean recall        & 0.874 \\
        Baseline $F_1$     & 0.000 \\
        $\Delta F_1$ vs baseline & +0.518 \\
        \bottomrule
        \end{tabular}
    \end{table}

    The model achieves ROC-AUC of 0.81, indicating good discriminative ability between risky and non-risky events based on features available before the execution stage. The high recall (0.87) means the model identifies most risky events before the execution stage, while the lower precision (0.38) reflects false positives: events that appear risky based on decision-stage features but resolve safely. This asymmetry is expected because not all tight decision-stage gaps lead to risky outcomes; follower adaptation and cutter acceleration can widen the gap during execution.
    ...
    \begin{table}[H]
        ...
        Feature & Importance \\
        \midrule
        Decision-stage THW$_{\min}$  & 0.314 \\
        Decision-stage DHW$_{\min}$  & 0.228 \\
        Intention-stage DHW$_{\min}$ & 0.120 \\
        Intention-stage THW$_{\min}$ & 0.100 \\
        Decision-stage TTC$_{\min}$  & 0.028 \\
        \bottomrule
        \end{tabular}
    \end{table}

    Decision-stage DHW and THW minima together account for over 54\% of the total importance, which confirms that gap acceptance in the decision stage is the strongest predictor of execution-stage risk. Among the SFC features, the two most informative bits (SFC positions 13 and 8) contribute modestly, suggesting that specific occupancy configurations carry additional predictive information.
    ...
    \begin{table}[H]
        ...
        \begin{tabular}{@{}lrrr@{}}
        \toprule
        Feature set & $n$ features & Mean AUC & Mean $F_1$ \\
        \midrule
        All (kinematic + SFC) & 34 & 0.810 & 0.518 \\
        Kinematic only        & 18 & 0.816 & 0.521 \\
        SFC only              & 16 & 0.613 & 0.344 \\
        \bottomrule
        \end{tabular}
    \end{table}

    Kinematic features alone achieve comparable performance (AUC 0.816), while SFC features alone still achieve AUC 0.613, well above chance (0.50). The spatial context signature carries meaningful information about future risk even without scalar speed or gap information. The SFC signature encodes which surrounding cells are occupied during the decision stage, showing that traffic configuration provides an independent risk signal.
...
% ==============================================================================
    ...
    \textbf{RQ4 (predictive value):} Decision-stage features predict execution-stage risk with AUC 0.81. SFC context features alone achieve AUC 0.61, which shows that spatial context signatures carry independent predictive information beyond scalar indicators.
...
Verified context signatures & 99.49\% row agreement; 7 archetypes; peak risk ratio 1.32 & Compact, reversible descriptors supporting scene retrieval and grouping. \\
Predictive evaluation & AUC 0.81 (all); AUC 0.61 (SFC alone) & Features carry predictive value; SFC adds independent signal. \\
...
\bottomrule
    ...
    \textbf{Construct validity:} Risk is operationalised via a THW threshold; alternative definitions (TTC, DRAC, post-encroachment measures) would label different event subsets. The threshold sensitivity analysis in Section~\ref{sec:drac-results} makes this dependence explicit. Additionally, highD lacks wider upstream/downstream context that may influence lane-change decisions~\cite{wanggao2025highd}.

    \textbf{Internal validity:} Reconstruction depends on lane-marking accuracy and coordinate conventions. Errors in these inputs can shift the onset $t_0$ and bias indicators. This is mitigated through geometry model selection against dataset-provided indicators and independent reconstruction variants that agree closely with the reference path.

    \textbf{External validity:} Results are based on \textit{highD} highway recordings and may not transfer to urban driving, ramp merges, or different sensing setups. Cross-dataset validation is identified as a priority for future work.

    \textbf{Conclusion validity:} The thesis reports descriptive differences, sensitivity analyses, and cross-validated predictions rather than causal claims about driver behaviour.
...
% ==============================================================================
    ...
    Three lines of evidence demonstrate that the contribution extends beyond reconstruction accuracy. First, multi-indicator analysis shows that THW, TTC, and DRAC capture different severity aspects, with risk interpretation depending on indicator choice and interaction phase. Second, decision-stage features predict execution-stage risk with AUC 0.81, and SFC features alone achieve AUC 0.61, showing that the framework produces features with predictive value. Third, exploratory context-archetype analysis shows that the SFC signatures support interpretable scene grouping linked to differential risk outcomes.
...
% ==============================================================================
    ...
    \begin{thebibliography}{99}
        ...
        \newblock DOI: \url{https://doi.org/10.1109/69.908985}.

        \bibitem{archer2005indicators}
        J.~Archer.
        \newblock Indicators for traffic safety assessment and prediction and their application in micro-simulation modelling: A study of micro-simulation modelling for the evaluation of safety effects of ITS.
        ...
        \bibitem{liu2021intention}
        Z.-Q.~Liu, M.-C.~Peng, and Y.-C.~Sun.
        \newblock Estimation of driver lane change intention based on the LSTM and Dempster--Shafer evidence theory.
        \newblock \emph{Journal of Advanced Transportation}, 2021:8858902, 2021.
        \newblock DOI: \url{https://doi.org/10.1155/2021/8858902}.

        \bibitem{yang2019lanechange}
        M.~Yang, X.~Wang, and M.~Quddus.
        \newblock Examining lane change gap acceptance, duration and impact using naturalistic driving data.
        \newblock \emph{Transportation Research Part C: Emerging Technologies}, 104:317--331, 2019.
        \newblock DOI: \url{https://doi.org/10.1016/j.trc.2019.05.024}.
...
\bottomrule
  ...
  \item \texttt{XX\_recordingMeta.csv} (recording metadata, including lane markings).
    ...
    Quantitative agreement metrics provide an overall assessment of reconstruction quality, but they do not on their own reveal whether failures are systematic (e.g., due to direction conventions, onset misalignment, or lane-boundary interpretation). To support qualitative verification, we use an interactive inspection dashboard that synchronises (i) aerial video playback and track overlays, (ii) the currently selected cut-in event and its cutter--follower pair, and (iii) the derived stage window, indicators, and context signature for the current frame.
    ...
    The dashboard was used to inspect representative cases and extremes (e.g., lowest-TTC events) and to verify that reconstructed relations and occupancy signatures correspond to the visible scene. It served as a support tool for verification and communication rather than as a standalone thesis contribution: it was not used to manually correct events or labels, and all reported results are produced by the framework's deterministic components.

    \begin{figure}[H]
        \centering
        \safeincludegraphics[width=\textwidth]{fig_gui_command_deck.png}
        \caption{Interactive inspection dashboard used for qualitative verification. The dashboard is based on a modified version of the official \textit{highD} visualisation tools and was extended with cut-in-specific overlays, event selection, and stage/context displays.}
