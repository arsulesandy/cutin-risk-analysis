  ...
  \item rows: \{ahead of the cutter, alongside the cutter, behind the cutter\}.
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[
            ...
            % scene
            \node[paneltitle, text width=3.2cm] at (1.7,4.3) {Local traffic scene};
            \draw[stagebox] (0.1,1.0) rectangle (3.3,3.25);
            \node[align=center, text width=2.4cm] at (1.7,2.15) {Cutter-centred\\local scene};

            % arrow 1
            \draw[arr] (3.7,2.15) -- (4.5,2.15);

            % 3x3 grid
            \node[paneltitle, text width=4.0cm] at (7.0,4.55) {3$\times$3 semantic occupancy grid};
            ...
            \node[font=\scriptsize, align=center, text=accent, text width=1.9cm] at (9.65,3.35) {mirror\\normalise};

            % 4x4 grid
            \node[paneltitle, text width=4.3cm] at (12.9,4.85) {Embed into fixed 4$\times$4 grid};
            ...
            \node[chip, anchor=west] at (10.75,0.02) {shaded cells = fixed padding};

            % arrow 3
            \draw[arr] (15.2,2.15) -- (16.1,2.15);
            \node[font=\scriptsize, align=center, text=accent, text width=1.7cm] at (15.65,3.35) {Hilbert\\order};

            % code boxes
            \node[paneltitle, text width=4.4cm] at (18.4,4.45) {Hilbert traversal $\rightarrow$ 16-bit code};
            \draw[stagebox] (16.5,2.7) rectangle (20.3,3.55);
            \node[align=center] at (18.4,3.12) {bits: $b_{15}\,\dots\,b_0$};
            \draw[stagebox] (16.5,1.35) rectangle (20.3,2.2);
            \node[align=center] at (18.4,1.77) {hex: e.g., \texttt{0x3A7F}};
            \node[font=\scriptsize, align=left, text width=4.4cm] at (18.45,0.35) {Compact, locality-preserving representation for fast matching and grouping.};
        \end{tikzpicture}%
        }
