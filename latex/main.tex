  ...
  \item rows: \{ahead of the cutter, alongside the cutter, behind the cutter\}.
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[x=0.9cm,y=0.9cm,>=Latex,font=\small]
            % scene
            \draw[rounded corners=2pt, line width=0.9pt] (0,0.2) rectangle (2.7,2.4);
            \node[align=center] at (1.35,1.3) {Local traffic scene\\around the cutter};

            % arrow 1
            \draw[-{Latex[length=3.2mm,width=2.0mm]}, line width=1.0pt] (2.9,1.3) -- (3.8,1.3);

            % 3x3 grid
            \node[font=\bfseries] at (5.4,3.0) {3$\times$3 semantic grid};
            \draw[line width=0.9pt] (4.0,0.0) rectangle (6.8,2.8);
            \foreach \x in {4.93,5.87} {\draw[line width=0.8pt] (\x,0.0) -- (\x,2.8);}
            \foreach \y in {0.93,1.87} {\draw[line width=0.8pt] (4.0,\y) -- (6.8,\y);}
            \fill[gray!20] (4.93,0.93) rectangle (5.87,1.87);
            \node[font=\scriptsize] at (5.40,1.40) {cutter};
            \node[font=\scriptsize] at (4.47,3.28) {left};
            \node[font=\scriptsize] at (5.40,3.28) {cutter's};
            \node[font=\scriptsize] at (5.40,3.02) {lane};
            \node[font=\scriptsize] at (6.33,3.28) {right};
            \node[font=\scriptsize, rotate=90] at (3.55,2.33) {ahead};
            \node[font=\scriptsize, rotate=90] at (3.55,1.40) {alongside};
            \node[font=\scriptsize, rotate=90] at (3.55,0.47) {behind};

            % arrow 2
            \draw[-{Latex[length=3.2mm,width=2.0mm]}, line width=1.0pt] (7.1,1.3) -- (8.0,1.3);
            \node[font=\scriptsize, align=center] at (7.55,2.25) {mirror-normalise};

            % 4x4 grid
            \node[font=\bfseries] at (10.6,3.0) {fixed 4$\times$4 embedding};
            \draw[line width=0.9pt] (8.8,0.0) rectangle (12.4,3.6);
            \foreach \x in {9.7,10.6,11.5} {\draw[line width=0.8pt] (\x,0.0) -- (\x,3.6);}
            \foreach \y in {0.9,1.8,2.7} {\draw[line width=0.8pt] (8.8,\y) -- (12.4,\y);}
            % active 3x3 region
            \draw[line width=1.1pt] (8.8,0.9) rectangle (11.5,3.6);
            % shaded padding cells
            \fill[gray!20] (8.8,0.0) rectangle (9.7,0.9);
            \fill[gray!20] (9.7,0.0) rectangle (10.6,0.9);
            \fill[gray!20] (10.6,0.0) rectangle (11.5,0.9);
            \fill[gray!20] (11.5,0.0) rectangle (12.4,0.9);
            \fill[gray!20] (11.5,0.9) rectangle (12.4,1.8);
            \fill[gray!20] (11.5,1.8) rectangle (12.4,2.7);
            \fill[gray!20] (11.5,2.7) rectangle (12.4,3.6);
            \node[font=\scriptsize, align=center] at (11.95,1.95) {fixed\\padding};

            % arrow 3
            \draw[-{Latex[length=3.2mm,width=2.0mm]}, line width=1.0pt] (12.7,1.3) -- (13.6,1.3);
            \node[font=\scriptsize, align=center] at (13.15,2.25) {Hilbert order};

            % code box
            \draw[rounded corners=2pt, line width=0.9pt] (13.8,0.55) rectangle (16.9,2.05);
            \node[align=center] at (15.35,1.30) {16-bit reversible\\context signature};
        \end{tikzpicture}
...
% ==============================================================================
  ...
  \item \textbf{Reconstruction path:} uses only trajectories and lane markings to produce the same outputs without using highD's derived relational fields as inputs.
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[
            font=\small,
            block/.style={draw, rounded corners=2pt, line width=1.05pt, minimum height=12mm, text width=3.9cm, align=center, inner sep=6pt},
            inputblock/.style={block, text width=3.6cm},
            line/.style={-{Latex[length=3.8mm,width=2.4mm]}, line width=1.10pt},
            group/.style={draw, rounded corners=4pt, dashed, line width=0.9pt, inner sep=7pt}
        ]
            \node[inputblock] (traj) {Trajectories and vehicle geometry\\$(x,y)$, speeds, dimensions};
            \node[inputblock, right=14mm of traj] (lanemeta) {Lane markings\\from recording metadata};
            \node[inputblock, right=14mm of lanemeta] (reflabels) {highD reference labels\\lane and neighbour ids};

            \node[block, below=13mm of traj] (lane) {Lane inference\\(XY-lane)};
            \node[block, below=9mm of lane] (neigh) {Leader/follower reconstruction\\within each inferred lane};
            \node[block, below=9mm of neigh] (events) {Lane-change detection\\and cut-in extraction};
            \node[block, below=9mm of events] (features) {Pair-specific indicators\\and context signatures};

            \node[block, below=23mm of reflabels] (reference) {Reference relations,\\events, and signatures};

            \path (features) -- (reference) coordinate[midway] (midout);
            \node[block, text width=4.8cm, below=17mm of midout] (eval) {Agreement evaluation\\frame level, event level,\\and signature level};

            \draw[line] (traj) -- (lane);
            \draw[line] (lanemeta) |- (lane);
            \draw[line] (lane) -- (neigh);
            \draw[line] (neigh) -- (events);
            \draw[line] (events) -- (features);
            \draw[line] (reflabels) -- (reference);
            \draw[line] (features.east) -| (eval.west);
            \draw[line] (reference.west) -| (eval.east);

            \node[group, fit=(traj)(lanemeta)(lane)(neigh)(events)(features),
                  label={[font=\bfseries]above:Minimal-input reconstruction path}] {};
            \node[group, fit=(reflabels)(reference),
                  label={[font=\bfseries]above:Reference-label path}] {};
            \node[group, fit=(eval),
                  label={[font=\bfseries]above:Evaluation boundary}] {};
        \end{tikzpicture}
...
\bottomrule
    ...
    To highlight how group differences evolve over time, Figure~\ref{fig:sfc-stage-diff} shows risk-minus-non-risk difference maps for the intention, decision, and execution stages using a consistent colour scale. The key point is not that risky and non-risky cut-ins form completely different scene types; rather, they share a common cut-in geometry and differ through comparatively small occupancy shifts in a few neighbouring cells. This is why the absolute maps look broadly similar and why the difference maps are more informative. Supplementary absolute maps for the intention and execution stages are provided in Appendix~\ref{app:occupancy-supp}.
    ...
    \begin{figure}[H]
        ...
        \caption{Decision-stage occupancy maps. Top: absolute mean occupancy for non-risk and risky events. Bottom: risk-minus-non-risk difference map, which is the main analytical view because it highlights the cells where occupancy frequency shifts between the two groups.}
    ...
    \begin{figure}[H]
        ...
        \caption{Stage-wise risk-minus-non-risk occupancy differences for the intention, decision, and execution stages, shown separately to make the small cell-wise shifts easier to read.}
        \label{fig:sfc-stage-diff}
    \end{figure}

    In practical terms, these plots should be read as subtle but structured differences in local traffic context. They suggest that the risky group is associated not with an entirely separate scene family, but with small changes in how tightly neighbouring cells around the cutter are occupied during the lead-up to the cut-in. In the decision stage, the most meaningful pattern is a modest tightening around the target-lane gap rather than a wholesale change of scene type, which is consistent with risky cut-ins being executed in more constrained surrounding traffic.
    ...
    \begin{figure}[H]
        \centering
        \safeincludegraphics[width=\textwidth]{archetype_heatmaps.png}
        \caption{Seven decision-stage context archetypes derived from mirrored binary SFC signatures. After mirror normalisation, the right column corresponds to the target lane, so each panel visualises one canonical cut-in scene family. The figure is intended to show recurring occupancy templates rather than exact cell-wise effect sizes; the quantitative risk comparison for these archetypes is given in Figure~\ref{fig:sfc-archetype-risk}.}
...
% ==============================================================================
    ...
    \paragraph{Transfer beyond highD.} Although this thesis evaluates only highD, the method is structured to make transfer relatively concrete. A second dataset would mainly require (i) a mapping from its road description to lane boundaries, (ii) harmonisation of coordinate conventions and vehicle reference points, and (iii) scenario filters suited to the road type. The onset definition, pair-consistent indicator computation, and SFC encoding can otherwise remain unchanged. The thesis therefore does not prove cross-dataset transfer, but it reduces transfer to a clearly scoped engineering task rather than a conceptual redesign.
...
% ==============================================================================
    ...
    Although the evaluation is confined to highD, transfer to other trajectory datasets appears technically plausible because the method depends on generic ingredients -- positions, vehicle geometry, and lane boundaries -- rather than highD-specific neighbour columns. Demonstrating such transfer empirically remains future work.
...
\bottomrule
  ...
  \item TTC is DHW divided by closing speed when the follower is gaining on the cutter; otherwise TTC is treated as infinite.
    ...
    Figure~\ref{fig:sfc-decision-grids} in Chapter~\ref{chap:results} presents the decision-stage occupancy maps in the main text because that stage is the most relevant pre-onset comparison. Figures~\ref{fig:sfc-supp-intention} and~\ref{fig:sfc-supp-execution} provide the corresponding absolute maps for the intention and execution stages at a larger size. These appendix figures are supplementary context views; the main analytical comparison remains the difference maps in Figure~\ref{fig:sfc-stage-diff}.

    \begin{figure}[H]
        \centering
        \safeincludegraphics[width=0.46\textwidth]{intention_nonrisk.png}\hfill
        \safeincludegraphics[width=0.46\textwidth]{intention_risk.png}
        \caption{Supplementary absolute occupancy maps for the intention stage (non-risk vs risky).}
        \label{fig:sfc-supp-intention}
    \end{figure}

    \begin{figure}[H]
        \centering
        \safeincludegraphics[width=0.46\textwidth]{execution_nonrisk.png}\hfill
        \safeincludegraphics[width=0.46\textwidth]{execution_risk.png}
        \caption{Supplementary absolute occupancy maps for the execution stage (non-risk vs risky).}
        \label{fig:sfc-supp-execution}
    \end{figure}
