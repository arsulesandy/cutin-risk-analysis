...
% --- Abstract ---
    ...
    The framework is evaluated on the \textit{highD} dataset, where highD lane and neighbour identifiers are used only as reference labels and not as method inputs. Across recordings 01--60, same-lane predecessor/follower reconstruction with inferred lanes reaches mean accuracy 0.999810/0.999811, and cut-in events detected from reconstructed relations match the reference-label events with $F_1 = 0.998938$. The detector extracts 10,026 cut-ins, of which 10,023 remain after stage-window validity checks. For 1,421,980 stage rows, reconstructed and reference context signatures agree exactly in 99.49\% of cases, and the SFC encoding is additionally verified through exhaustive round-trip testing over all $2^{16}$ code values. The mirrored decision-stage signatures yield seven stable context archetypes; the most risk-enriched archetype shows a 1.32$\times$ higher execution-stage THW risk prevalence than the overall event set.

    Beyond reconstruction accuracy, the thesis demonstrates predictive value: a leave-one-recording-out cross-validation experiment shows that decision-stage features predict execution-stage THW risk with ROC-AUC 0.81. SFC context features alone achieve AUC 0.61, confirming that the spatial context signatures carry independent risk-relevant information beyond scalar indicators. The addition of DRAC as a deceleration-based indicator confirms that the framework's pair-consistent computation extends naturally to complementary safety measures, and that most detected cut-ins (99.9\%) involve routine gap acceptance rather than hard-braking situations.
...
% ==============================================================================
  ...
  \item a \textbf{context-signature layer} that encodes the surrounding traffic configuration into a reversible 16-bit integer code using a Hilbert space-filling-curve traversal, enabling compact, direction-normalised context comparison.
    \end{enumerate}
  ...
  \item \textbf{Predictive evaluation:} a LORO-CV experiment showing that decision-stage features predict execution-stage THW risk with ROC-AUC 0.81, with SFC context features alone achieving AUC 0.61. This demonstrates that the framework produces features with genuine predictive value beyond descriptive agreement.
...
% ==============================================================================
    ...
    To assess whether the framework produces features with genuine predictive value, we test whether features from the intention and decision stages, including SFC context signatures, predict execution-stage risk using leave-one-recording-out cross-validation (LORO-CV). An ablation study isolates the contribution of SFC features versus kinematic indicators.
...
\bottomrule
    ...
    These parameters are informed by literature on staged lane-change processes~\cite{wang2019cutin,zhou2016riskperception} and are reported to support reproducibility.
...
\ForEach{frame $t$ in the analysis window around $t_0$}{
    ...
    We compute DRAC for the explicit cutter--follower pair in the execution stage using the formula defined in Section~\ref{sec:drac-theory}.  The implementation reuses the same pair-consistent longitudinal states (positions, speeds, and DHW) already computed for TTC and THW, ensuring that all indicators describe the same interaction pair.  DRAC is reported alongside the existing indicators to assess braking demand complementarily to gap acceptance (THW) and collision timing (TTC).

    \section{Predictive evaluation: decision-stage risk forecasting}
    \label{sec:risk-prediction-method}
    Beyond descriptive agreement, we evaluate the \emph{predictive value} of the framework's reconstructed features.  Specifically, we test whether features available \emph{before} the cut-in is established (intention and decision stages) can predict execution-stage THW risk (THW$_{\min} < \SI{0.7}{\second}$).
...
\bottomrule
    ...
    Searching $K=3,\dots,8$ with $k$-means, the best solution is $K=7$, with silhouette score 0.498 and mean stability ARI 0.992 across repeated initialisations. The resulting archetypes remain reasonably balanced, spanning 6.0\%--30.7\% of all 10,023 validated cut-ins. After mirror normalisation, all lane changes are represented in a common canonical direction, so the right column in Figure~\ref{fig:sfc-archetypes} corresponds to the target lane for interpretation.

    Figure~\ref{fig:sfc-archetypes} shows the seven archetypes, and Figure~\ref{fig:sfc-archetype-risk} compares their execution-stage THW risk prevalence. The most risk-enriched archetype (A4, 10.27\% of events) reaches 26.63\% risky outcomes versus 20.13\% overall (risk ratio 1.32) and has median execution THW \SI{0.975}{\second}. Its mean grid is dominated by occupied target-lane preceding and following cells, indicating insertion into a relatively constrained target-lane corridor. Lower-risk archetypes such as A2 and A5 remain below the overall prevalence (17.18\% and 17.51\%, respectively). This exploratory analysis demonstrates that the SFC code supports interpretable unsupervised grouping of cut-in scenes based on decision-stage context.
    ...
    The model achieves ROC-AUC of 0.81, indicating good discriminative ability between risky and non-risky events based on features available before the execution stage.  The high recall (0.87) means that the model identifies most risky events before the execution stage, while the lower precision (0.38) reflects false positives: events that appear risky based on decision-stage features but resolve safely.  This asymmetry is expected because not all tight decision-stage gaps lead to risky outcomes, since follower adaptation and cutter acceleration can widen the gap during execution.
    ...
    Kinematic features alone achieve comparable performance (AUC 0.816), while SFC features alone still achieve AUC 0.613, well above chance (0.50).  This result means that the spatial context signature, even without any scalar speed or gap information, carries meaningful information about future risk.  The SFC signature encodes which surrounding cells are occupied during the decision stage, and the result demonstrates that traffic configuration provides an independent risk signal.
...
% ==============================================================================
    ...
    \textbf{RQ1 (reconstruction accuracy):} Same-lane neighbour reconstruction from geometry reaches mean accuracy $>$0.9998 against highD reference labels. Cut-in detection achieves $F_1 = 0.999$, confirming that the minimal-input path reproduces the reference structure.
    ...
    \textbf{RQ4 (predictive value):} Decision-stage features predict execution-stage risk with AUC 0.81. SFC context features alone achieve AUC 0.61, demonstrating that spatial context signatures carry independent predictive information beyond scalar indicators.
    ...
    This design has three practical implications. First, explicit cutter--follower pairing ensures that safety indicators describe the intended interaction rather than a generic predecessor relation. Second, the multi-indicator results (THW, TTC, DRAC) confirm that risk interpretation is indicator-dependent and phase-dependent~\cite{zhou2016riskperception,wang2021ssm}, reinforcing the need for transparent indicator choice. Third, the LORO-CV prediction experiment demonstrates that the framework produces features with genuine predictive value: achieving AUC 0.81 from features available before the execution stage means the outputs support early risk identification, not merely reference-label reproduction. The ablation showing AUC 0.61 for SFC features alone further confirms that the context signatures encode risk-relevant spatial information beyond scalar indicators.
...
Predictive evaluation & AUC 0.81 (all); AUC 0.61 (SFC alone) & Features carry genuine predictive value; SFC adds independent signal. \\
...
% ==============================================================================
    ...
    Three lines of evidence demonstrate that the contribution extends beyond reconstruction accuracy. First, multi-indicator analysis shows that THW, TTC, and DRAC capture different severity aspects, with risk interpretation depending on indicator choice and interaction phase. Second, decision-stage features predict execution-stage risk with AUC 0.81, and SFC features alone achieve AUC 0.61, confirming that the framework produces features with genuine predictive value. Third, exploratory context-archetype analysis shows that the SFC signatures support interpretable scene grouping linked to differential risk outcomes.

    The result is a reproducible, auditable pipeline for cut-in analysis that can serve as a basis for cross-dataset transfer, richer predictive models, and downstream applications requiring pair-consistent interaction features.
