...
% --- Abbreviations ---
    ...
    \begin{tabularx}{\textwidth}{@{}l>{\raggedright\arraybackslash}X@{}}
        ...
        highD & Drone-based highway trajectory dataset used in this study~\cite{highd_2018} \\
...
% ==============================================================================
    ...
    \begin{table}[htbp]
        \centering
        \caption[Comparison with related lane-change and cut-in studies]{Comparison of the framework presented in this thesis with related lane-change and cut-in studies. \checkmark\ = supported, -- = not addressed.}
...
\bottomrule
    ...
    \begin{figure}[H]
        ...
        \caption[Schematic layout of the interactive inspection dashboard]{Schematic layout of the interactive inspection dashboard used for qualitative review. The implemented dashboard is based on a modified version of the official \textit{highD} visualisation tools and was extended with cut-in-specific overlays, event selection, and stage/context displays.}
    ...
    \begin{figure}[H]
        ...
        \caption[Example exiD dashboard view used in the exploratory extension]{Example exiD dashboard view used in the exploratory extension. The same event-centred inspection workflow was reused for exiD scenes, with dataset-specific background rendering but unchanged cut-in review logic.}
  ...
  \item adapt the event-persistence parameters to NGSIM's \SI{10}{\hertz} rate: 10 stable frames before the lane change, 10 stable frames after, a 20-frame follower search window, a 6-frame maximum relation delay, a 5-frame minimum relation duration, and a 10-frame precheck window.
    ...
    \begin{figure}[tbp]
        ...
        \caption[Exploratory NGSIM feasibility probe on the two freeway subsets]{Exploratory NGSIM feasibility probe on the two freeway subsets. Bars show detected lane changes and cut-in candidates after harmonisation; labels above the cut-in bars show cut-in candidates as a share of detected lane changes. The figure is reported as a scoping artefact only and is not interpreted as cross-dataset validation performance.}
        \label{fig:ngsim-feasibility}
    \end{figure}

    \begin{table}[tbp]
        \centering
        \caption[Summary of the NGSIM feasibility probe]{Summary of the NGSIM feasibility probe. Negative-gap share denotes candidate events with at least one frame where reported \texttt{Space\_Headway} minus cutter length became negative; it is used only as a harmonisation stress indicator, not as a behavioural result.}
        \label{tab:ngsim-feasibility}
        {\small
        \renewcommand{\arraystretch}{1.15}
            \begin{tabularx}{0.82\textwidth}{@{}>{\raggedright\arraybackslash}X r r@{}}
\toprule
Metric & I-80 & US-101 \\
\midrule
Raw rows & 4,566,387 & 4,802,933 \\
Duplicates removed & 0 & 704,000 \\
Track instances & 5,678 & 6,101 \\
Lane changes & 2,647 & 2,780 \\
Cut-in candidates & 2,530 & 1,919 \\
Cut-in ratio & 95.6\% & 69.0\% \\
Median min THW & \SI{2.05}{\second} & \SI{1.60}{\second} \\
Finite TTC share & 67.5\% & 54.3\% \\
Negative-gap share & 4.0\% & 2.4\% \\
\bottomrule
            \end{tabularx}}
    \end{table}
