...
\bottomrule
  ...
  \item \textbf{Reference agreement:} for cut-in stage frames, we compute a reference occupancy grid from highD surrounding-vehicle ids and compare its SFC code to the minimal-input code computed from inferred lanes and geometry. Exact matches indicate identical neighbour occupancy under the two definitions; non-matches localise differences to specific cells via Hamming distance.
    ...
    \section{Exploratory cross-dataset feasibility check (NGSIM)}
    \label{sec:ngsim-feasibility}
    To test whether the framework is conceptually tied to \textit{highD} or mainly to engineering choices in preprocessing, we ran a deliberately limited transferability check on the public NGSIM trajectory dataset~\cite{ngsim_2016}. We restricted the analysis to the two freeway subsets, US-101 and I-80, because the arterial subsets (Lankershim Boulevard and Peachtree Street) mix intersection movements, turning traffic, and non-highway lane codes that would confound a highway cut-in study.

    This add-on was not meant to reproduce the full highD evaluation. The question was narrower: can the lane-change and cut-in mining core be moved to another trajectory source with limited adaptation work? For that reason, the NGSIM results are not treated as a second benchmark and are not combined with the main highD statistics.

    The NGSIM prototype processes one location at a time, removes exact duplicate rows, splits repeated source ids when \texttt{Global\_Time} jumps by more than one second, remaps \texttt{Preceding} and \texttt{Following} to the split track instances, and rescales the event-mining thresholds from the highD setting (\SI{25}{\hertz}) to NGSIM's \SI{10}{\hertz}. The feasibility check stops at explicit cutter--follower event mining and pairwise risk summaries. We do not attempt full context-signature validation because NGSIM does not provide highD-style left/right surrounding-vehicle identifiers.

    \begin{table}[H]
        \centering
        \caption{Exploratory NGSIM freeway feasibility summary. Percentages are calculated over the candidate cut-ins extracted in each subset.}
        \label{tab:ngsim-feasibility}
        {\small
        \setlength{\tabcolsep}{4pt}
            \begin{tabular}{@{}lrrrrrrrr@{}}
\toprule
Location & Raw rows & Dup. removed & Tracks & Lane chg. & Cut-ins & Cut-ins / LC & TTC finite & Neg. gap \\
\midrule
I-80   & 4,566,387 & 0       & 5,678 & 2,647 & 2,530 & 95.6\% & 67.5\% & 4.0\% \\
US-101 & 4,802,933 & 704,000 & 6,101 & 2,780 & 1,919 & 69.0\% & 54.3\% & 2.4\% \\
\bottomrule
            \end{tabular}}
    \end{table}
    ...
    \begin{figure}[H]
        \centering
        \safeincludegraphics[width=0.72\textwidth]{ngsim_feasibility_counts.png}
        \caption{Lane-change and cut-in counts from the exploratory NGSIM freeway feasibility check. The labels above the cut-in bars show the cut-in-to-lane-change ratio. The figure demonstrates that the event-mining core transfers to another highway trajectory source, but it should not be read as a benchmark comparison against highD because the subsets differ in duration, geometry, and data quality.}
        \label{fig:ngsim-feasibility}
    \end{figure}

    Table~\ref{tab:ngsim-feasibility} and Figure~\ref{fig:ngsim-feasibility} show that both freeway subsets still produce sizeable event sets after explicit cleaning: 2,780 lane changes and 1,919 candidate cut-ins on US-101, and 2,647 lane changes and 2,530 candidate cut-ins on I-80. This is enough to show that the lane-change and cut-in logic is not tied only to highD. The high I-80 cut-in-to-lane-change ratio should be read only as prototype event yield under the chosen thresholds, not as a behavioural prevalence claim.

    At the same time, the feasibility check also shows why NGSIM should not be treated as a second full validation dataset without further work. First, the export quality is lower: the US-101 subset required removal of 704,000 exact duplicate rows before event mining. Second, NGSIM headways are front-centre based and the kinematics are noisier, consistent with prior observations that direct velocity and acceleration use in NGSIM is sensitive to positional noise~\cite{thiemann2008ngsim}. Third, the available neighbour information is weaker than in highD, which means the full surrounding-context branch cannot be evaluated in the same way. For these reasons, the NGSIM results are used only as exploratory transfer evidence, not as cross-dataset validation on the same level as the main highD study.
...
% ==============================================================================
    ...
    \paragraph{Transfer beyond highD.} The NGSIM feasibility check in Section~\ref{sec:ngsim-feasibility} makes the transfer question more concrete. After explicit cleaning, identity splitting, neighbour remapping, and threshold rescaling, both freeway subsets still yield sizeable lane-change and cut-in event sets. This suggests that transfer is not blocked by the event definition itself. At the same time, the exercise also shows that transfer is not free: duplicate rows, repeated source ids, front-centre headway definitions, and noisier kinematics become important engineering constraints as soon as the method leaves the more curated highD setting. The framework therefore reduces cross-dataset transfer to a bounded engineering task, but it does not remove dataset differences.
...
\bottomrule
    ...
    \begin{enumerate}[leftmargin=1.6em]
  \item \textbf{Full cross-dataset validation:} extending the current exploratory NGSIM check into a calibrated multi-dataset study on inD, rounD, exiD, or cleaner NGSIM subsets, including coordinate harmonisation, kinematic smoothing, and representation-level validation beyond event mining.
...
% ==============================================================================
    ...
    A small exploratory NGSIM freeway check further supports this claim: after explicit cleaning and identity handling, the same lane-change and cut-in mining logic still produces substantial event sets on US-101 and I-80. However, the exercise also makes the limitation clear. Cross-dataset transfer is constrained less by the event definition itself than by dataset quality, neighbour semantics, and coordinate conventions. A full multi-dataset validation therefore remains future work, even if basic transferability is now illustrated with one public secondary dataset.
