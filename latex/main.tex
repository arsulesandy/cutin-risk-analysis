...
% --- Abstract ---
    ...
    The framework is evaluated on the \textit{highD} dataset, where highD lane and neighbour identifiers are used only as reference labels and not as method inputs. Across recordings 01--60, same-lane predecessor/follower reconstruction with inferred lanes reaches mean accuracy 0.999810/0.999811, and cut-in events detected from reconstructed relations match the reference-label events with $F_1 = 0.998938$. The detector extracts 10,026 cut-ins, of which 10,023 remain after stage-window validity checks. For 1,421,980 stage rows, reconstructed and reference context signatures agree exactly in 99.49\% of cases, and the SFC encoding is additionally verified through exhaustive round-trip testing over all $2^{16}$ code values. As an additional exploratory analysis, the mirrored decision-stage signatures yield seven stable context archetypes; the most risk-enriched archetype shows a 1.32$\times$ higher execution-stage THW risk prevalence than the overall event set.

    The main contribution of the thesis is methodological rather than purely dataset-specific. Relation fields that are usually treated as fixed dataset columns become explicit intermediate outputs that can be reconstructed, audited, and reused for pair-consistent indicator computation and reversible context encoding. This makes the analysis reproducible, easier to inspect, and more transferable to datasets that provide trajectories and lane-boundary information but not neighbour relations directly.
...
% ==============================================================================
  ...
  \item \textbf{Method-evaluation perspective:} treat highD's derived fields as \emph{reference labels} , and evaluate a method that must work using only lower-level inputs such as trajectories and lane markings.
    ...
    \section{Limitations of matching highD alone}
    High agreement against highD is important, but it is not by itself a sufficient thesis contribution. If the method only reproduces the dataset's provided relations, the work may still appear to be a careful reconstruction exercise. The thesis therefore examines the analytical consequences of reconstructing the relevant pair explicitly. In the present case, this means defining a cut-in as a cutter--follower event, computing DHW, THW, and TTC for that pair, and showing how the interpretation changes across stages and thresholds. In this way, reconstruction accuracy is extended into a reusable analysis pipeline rather than remaining a matching exercise alone.

    \section{Key idea and novelty}
    The core design of \FrameworkName{} is to reconstruct lane and same-lane neighbour relations from minimal inputs, use those reconstructed relations to define cut-ins explicitly, and encode local context in a reusable compact form. The framework therefore has three connected parts:
  ...
  \item a \textbf{context-signature layer} that turns the surrounding traffic configuration into a compact, reversible integer code using a Hilbert space-filling-curve traversal.
    ...
    The key design choice is the evaluation boundary. highD lane and neighbour identifiers are used only as reference labels and never as hidden inputs to the method. This design permits direct assessment of the minimal-input path and keeps the downstream cut-in analysis independent of dataset-specific relation columns. The individual ingredients are known in the literature; the contribution here is to integrate them into a coherent pipeline for cut-in analysis and to evaluate that pipeline explicitly against reference labels.
  ...
  \item \textbf{Explicit cutter--follower cut-in definition:} cut-ins are detected as explicit interacting pairs with a reproducible onset definition ($t_0$), thereby making feature extraction consistent and auditable.
...
% ==============================================================================
    ...
    \section{Rationale for pair-specific indicator computation}
...
% ==============================================================================
    ...
    We included papers that address highway lane changes/cut-ins, define or review surrogate safety measures, use \textit{highD} (or closely related datasets) for scenario extraction, or propose compact spatial context representations for interaction analysis. We complemented keyword searching with backward and forward checking on core papers such as the \textit{highD} dataset paper and the main SSM review papers.

    \section{Surrogate safety measures and validation}
    Surrogate safety measures (SSMs) are used because crashes are rare in naturalistic data. In this thesis, the main indicators of interest are TTC, THW, and DHW. Recent reviews and validation-oriented studies emphasise that SSM values depend strongly on modelling assumptions, measurement noise, and traffic state, and therefore need careful interpretation \cite{wang2021ssm,johnsson2021validation}.
...
% ==============================================================================
    ...
    The evaluation in this thesis serves two purposes. First, it establishes whether the minimal-input reconstruction reproduces highD's relational structure with sufficient accuracy. Second, it evaluates the analytical value of the framework once that correctness baseline has been established. For that reason, the evaluation is reported at four levels:
    ...
    To complement the quantitative metrics, we performed qualitative spot checks using an interactive dashboard that synchronises video playback with reconstructed relations, event timing, and context signatures. This supported verification that the reported metrics correspond to visually plausible interactions and also provided illustrative examples.
  ...
  \]
    ...
    \]
    This rule is not presented as a universal definition of risk. The threshold of
    \SI{0.7}{\second} was chosen as a deliberately strict cut-off for a very short
    accepted gap. Experimental work on time-headway perception discusses much larger
    highly critical guidance values around \SIrange{1.8}{2.0}{\second}, while experimental work on perceived criticality explicitly evaluates \SI{0.7}{\second} as a short time headway \cite{tscharn2018thw}. We choose
    execution-stage THW for this descriptive label because it remains defined after
    the new follower relation has been established, even when TTC becomes infinite
    under non-closing conditions. The thesis therefore reports threshold sensitivity to
    keep the operational choice explicit rather than implicit
    \cite{wang2021ssm,zhou2016riskperception}.
...
% ==============================================================================
  ...
  \item \textbf{Reconstruction path (system under evaluation):} uses only $x,y$ trajectories and lane markings to infer lane indices (XY-lane), reconstruct same-lane neighbours, and then produce the same event and signature outputs \emph{without} using highD neighbour columns as inputs.
    ...
    For clarity, the framework is organised into descriptive macro-phases rather than numbered processing steps. The reconstruction path can be interpreted as a \emph{sequence of deterministic processing stages}: each component applies explicit acceptance criteria (e.g., lane stability, relation duration, stage-window validity) and emits traceable outputs.
...
\bottomrule
    ...
    \begin{figure}[H]
        ...
        \caption{Interactive inspection dashboard used for qualitative verification and illustration. The dashboard is based on a modified version of the official \textit{highD} visualisation tools and was extended in this thesis with cut-in-specific overlays, event selection, and stage/context displays.}
...
\Return no event\;
    ...
    The framework keeps \emph{event extraction} separate from \emph{risk labelling}. The settings used in the final run are design choices, but they are informed by previous work rather than selected arbitrarily.
  ...
  \item \textbf{Pair-consistent indicator computation.} DHW, THW, and TTC are computed for the explicit cutter--follower pair, not for a generic dataset predecessor. This is important because recent lane-change risk models on \textit{highD} rely on interaction-specific features and time-to-lane-change rather than on anonymous vehicle states \cite{shangguan2022,liu2025multitask}.
  \item \textbf{Phase-aware interpretation.} Following Zhou and Itoh, TTC is especially informative when evaluating the targeted rear vehicle before or around insertion, while THW remains informative once the pair has been established in the target lane \cite{zhou2016riskperception}. For that reason, the thesis reports all three measures but uses execution-stage THW only as a transparent descriptive risk label.
    ...
    These choices are not claimed to be universally best. Their purpose is to make the cut-in definition explicit, auditable, and amenable to later sensitivity analysis.
    ...
    As an additional analysis of representational usefulness, we cluster event-level decision-stage context signatures after the core verification step. For each validated cut-in event, the mirror-normalised decision-stage 3$\times$3 occupancy grids are averaged over the frames in that stage, yielding a 9-dimensional vector of cell-wise occupancy probabilities. We then apply $k$-means clustering for $K=3,\dots,8$, assess internal cohesion with the silhouette score, and assess solution stability with the mean adjusted Rand index (ARI) across repeated random initialisations. This clustering is not used as a primary correctness criterion; it is reported only as an exploratory check of whether the verified SFC representation supports interpretable grouping of pre-outcome interaction contexts.
...
\ForEach{frame $t$ in the analysis window around $t_0$}{
    ...
    The framework is parameterised through a small set of explicit settings (e.g., stability thresholds and stage windows). Each run produces versioned outputs (tables and figures) that can be regenerated from the same raw data and settings. The thesis-facing claims in Chapters~\ref{chap:results}--\ref{chap:conclusion} rely on the all-recordings reconstruction, stage-feature, risk, and context-signature artifacts produced by the retained thesis pipeline. The intended entry point for reproducing the reported tables and figures is the project's reproducibility package or repository: \ArtifactURL.
...
\bottomrule
    ...
    Overall, 84.56\% of detected lane changes are classified as cut-ins under our definition (10,026 cut-ins out of 11,856 lane changes). This proportion should be read as the yield of the operational event definition used in this thesis, not as a general prevalence estimate for highway traffic.
    ...
    The high agreement reported above addresses the correctness question, but it does not by itself establish the analytical value of the framework. That value arises from making the event definition and the indicator interpretation explicit.
    ...
    This pattern is consistent with previous work showing that lane-change risk perception is stage dependent and that TTC and THW describe different parts of the interaction \cite{zhou2016riskperception,wang2021ssm}. In that sense, the contribution of \FrameworkName{} is not a claim that one threshold is universally correct. The contribution is that the framework reconstructs the relevant interaction pair, computes the indicators for that pair, and keeps the later label choice explicit.
...
% ==============================================================================
    ...
    \textbf{RQ2 (cut-in consistency):} Stable-block lane-change detection combined with follower pairing yields 10,026 extracted cut-ins from 11,856 lane changes across recordings 01--60, of which 10,023 remain after stage-window validity checks. When the full cut-in detector is run end-to-end on reconstructed relations, event matching against the reference-label path achieves $F_1 = 0.998938$. Beyond correctness, the explicit cutter--follower representation enables pair-consistent surrogate-measure computation and preserves an explicit event definition.

    \textbf{RQ3 (SFC encoding and verification):} A compact 3$\times$3 occupancy grid encoded into a 16-bit SFC signature provides an interpretable local context representation. The encoding is reversible by design and is validated by exhaustive round-trip checks. In the final configuration, reference-vs-minimal-input matching reaches 99.49\% exact row agreement over 1,421,980 verified rows (with 100\% event-level ANY-frame agreement), confirming that the reconstructed occupancy used for SFC encoding corresponds closely to the reference relations in highD once direction is normalised. The same representation also supports an exploratory unsupervised grouping: decision-stage context clustering yields seven archetypes, with the most constrained target-lane corridor archetype showing a 1.32$\times$ higher execution-stage THW risk prevalence than the overall set.

    \section{Interpretation and implications}
    The principal outcome of the thesis is methodological. The framework defines a clear system boundary: trajectories and lane markings serve as inputs, whereas highD relation fields serve as evaluation labels. This separation allows disagreements with dataset conventions to be analysed explicitly rather than inherited as hidden assumptions.

    The innovative step is relation-first analysis: lane ids and neighbour ids are treated as reconstructable intermediate products rather than immutable dataset facts. Although algorithmically modest, this shift is scientifically important because it separates observation, derived relation, event definition, and risk label into auditable layers. The result is a pipeline whose assumptions can be inspected, challenged, and transferred more readily than those of a pipeline that starts from hidden dataset-specific relation columns.

    This transferability still has conditions. The current method assumes trajectories are expressed in a consistent coordinate system and that lane-boundary metadata, or an equivalent road model, is available. The contribution is therefore not that the framework is assumption free, but that its assumptions are explicit and that their consequences can be measured against reference labels.
...
Verified context signatures & Reversible 16-bit Hilbert code; 99.49\% exact row agreement over 1,421,980 stage rows; exploratory decision-stage archetypes with peak risk ratio 1.32 & Provides compact, reversible context descriptors that support interpretable grouping of cut-in scenes rather than only average occupancy plots. \\
...
\bottomrule
    ...
    Explicit pairing matters because the safety-relevant interaction after a cut-in is the cutter and the new follower in the target lane. Dataset-provided indicators computed for a generic predecessor relation may be consistent with dataset conventions yet still correspond to a different interaction pair in a cut-in study. \FrameworkName{} addresses this by defining cut-ins as explicit cutter--follower events and computing DHW, THW, and TTC for that pair only.
    ...
    The SFC signatures complement the scalar indicators. Because the code is reversible, it does not replace interpretation with a black-box embedding. Instead, it provides a compact summary that can be decoded, checked, and then used for large-scale comparison, retrieval, or clustering of interaction patterns. The exploratory archetype analysis makes this concrete: a pre-outcome decision-stage grouping can be learned directly from the verified signatures, and the resulting scene families are meaningfully associated with later execution-stage THW risk.
    ...
    \section{Future work}
    Natural next steps include (i) extending the surrogate-measure layer with braking-demand or minimum-safe-deceleration criteria as complementary lane-change safety indicators, (ii) adding broader ablation studies over relevance-distance, pair-persistence, and stage-boundary criteria, ideally with manual review of disagreement cases, and (iii) testing whether the learned context archetypes transfer across datasets, road classes, or prediction tasks rather than remaining specific to \textit{highD}'s highway setting.
...
% ==============================================================================
    ...
    The thesis also demonstrates that the contribution extends beyond reconstruction accuracy. Once the cutter and follower are made explicit, DHW, THW, and TTC can be computed for the correct pair and interpreted in a stage-based way. The threshold sensitivity analysis and TTC coverage results show that subsequent risk interpretation depends strongly on indicator choice and phase.

    Overall, the contribution is a clear and reusable pipeline for cut-in analysis: minimal inputs, explicit interacting pair, verified context representation, and an evaluation setup that makes each step auditable and reproducible. The added exploratory archetype analysis further suggests that the context signatures support an interpretable scene grouping rather than only frame-level verification. This provides a solid basis for future retrieval, cross-dataset archetype transfer, or predictive use of cut-in scenarios.
