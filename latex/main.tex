...
\usepackage{graphicx}
\usepackage{xcolor}
\definecolor{lightfill}{gray}{0.94}
\usepackage{tikz}
...
% ==============================================================================
  ...
  \item a \textbf{context-signature layer} that encodes the surrounding traffic configuration into a reversible 16-bit integer code using a Hilbert space-filling-curve traversal for compact, direction-normalised context comparison.
    ...
    The framework is evaluated primarily on \textit{highD} highway recordings (drone-based trajectories at \SI{25}{\hertz}) using offline batch processing. To show at least one concrete example beyond \textit{highD} without expanding scope, the thesis also includes a small exploratory exiD extension on 10 selected recordings. This add-on is only meant to illustrate transferability of the event-mining and pairwise risk-summary path; it is not presented as a second benchmark study or as evidence equivalent to the main highD evaluation. The thesis does not aim to infer driver intent beyond observable trajectories, does not include simulation-based re-enactment, and does not claim that any single risk threshold is universally valid. The emphasis remains on reproducible scenario extraction, pair-consistent indicator computation, and reference-label evaluation on highD.
...
% ==============================================================================
    ...
    As a small supporting transfer check, the thesis also uses exiD~\cite{exid_2022}, another drone-based German highway trajectory dataset focused on highly interactive scenarios. exiD is not used as a second full benchmark here. Instead, a 10-recording subset is used only to test whether the explicit lane-change and cut-in mining path, together with compact pairwise risk summaries, can be adapted beyond the primary highD setting with limited dataset-specific glue code.

    \section{Surrogate safety measures}
    Because crashes are rare in naturalistic data, surrogate safety measures (SSMs) are widely used to assess interaction severity without requiring crash observations. The main indicators used in this thesis are time-to-collision (TTC), time headway (THW), distance headway (DHW), and the deceleration rate to avoid crash (DRAC). Recent reviews and validation studies emphasise that SSMs require careful interpretation and validation because their values depend on modelling assumptions and data quality~\cite{wang2021ssm,johnsson2021validation}. Deceleration-based indicators expressed in \si{\meter\per\second\squared}, such as DRAC, complement time-based measures; Archer~\cite{archer2005indicators} uses \SI{3.35}{\meter\per\second\squared} as a threshold to distinguish serious from non-serious conflicts, while Wang et al.~\cite{wang2021ssm} report DRAC thresholds in the range of \SIrange{3.0}{3.5}{\meter\per\second\squared} across the literature.
...
% ==============================================================================
    ...
    This study adopts a design-oriented empirical methodology. We implement a reconstruction-based framework and evaluate its outputs against reference labels provided by the \textit{highD} dataset. The evaluation is structured at four levels, each addressing a different aspect of the framework's validity. A separate exiD sidecar run is added only as a supporting exploratory transferability check; it does not replace or duplicate the four-level highD benchmark.
  ...
  \item \textbf{Recovery:} $[t_0+\SI{2}{\second},\, t_0+\SI{4}{\second}]$
    ...
    Figure~\ref{fig:stage-timeline} illustrates the four-stage alignment around $t_0$. Because \textit{highD} is recorded at \SI{25}{\hertz}, one frame corresponds to \SI{0.04}{\second}, and all frame counts in the implementation are converted to seconds using this rate.
  ...
  \item rows: \{ahead of the cutter, alongside the cutter, behind the cutter\}.
    ...
    Each cell is treated as binary occupancy ($1$ if at least one vehicle occupies the cell, else $0$). The 3$\times$3 grid is embedded in a fixed 4$\times$4 grid and encoded as a 16-bit integer using a locality-preserving space-filling-curve order (Hilbert order by default). Because the mapping is deterministic and reversible, we can decode a code back into a grid. Figure~\ref{fig:sfc-signature-overview} summarises the encoding pipeline from local traffic scene to reversible 16-bit signature.
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[x=1cm,y=1cm,>=Latex,font=\small]
            % scene
            \node[font=\bfseries, align=center, text width=2.9cm] at (1.4,3.9) {Local traffic scene};
            \draw[rounded corners=2pt, line width=0.9pt] (0.0,1.0) rectangle (2.8,3.0);
            \node[align=center, text width=2.2cm] at (1.4,2.0) {Around the cutter};

            % arrow 1
            \draw[-{Latex[length=3.2mm,width=2.0mm]}, line width=1.0pt] (3.2,2.0) -- (4.1,2.0);

            % 3x3 grid
            \node[font=\bfseries, align=center, text width=3.6cm] at (6.1,4.3) {3$\times$3 semantic occupancy grid};
            \draw[line width=0.9pt] (4.6,0.7) rectangle (7.6,3.7);
            \foreach \x in {5.6,6.6} {\draw[line width=0.8pt] (\x,0.7) -- (\x,3.7);}
            \foreach \y in {1.7,2.7} {\draw[line width=0.8pt] (4.6,\y) -- (7.6,\y);}
            \fill[gray!20] (5.6,1.7) rectangle (6.6,2.7);
            \node[font=\scriptsize, align=center] at (6.1,2.2) {cutter};
            \node[font=\scriptsize, anchor=east] at (4.35,3.2) {ahead};
            \node[font=\scriptsize, anchor=east] at (4.35,2.2) {alongside};
            \node[font=\scriptsize, anchor=east] at (4.35,1.2) {behind};
            \node[font=\scriptsize, align=center] at (5.1,0.2) {left};
            \node[font=\scriptsize, align=center, text width=1.1cm] at (6.1,0.18) {cutter\\lane};
            \node[font=\scriptsize, align=center] at (7.1,0.2) {right};

            % arrow 2
            \draw[-{Latex[length=3.2mm,width=2.0mm]}, line width=1.0pt] (8.0,2.0) -- (9.0,2.0);
            \node[font=\scriptsize, align=center, text width=1.8cm] at (8.5,3.15) {mirror-\\normalise};

            % 4x4 grid
            \node[font=\bfseries, align=center, text width=3.8cm] at (11.6,5.0) {Embed into fixed 4$\times$4 grid};
            \draw[line width=0.9pt] (9.6,0.4) rectangle (13.6,4.4);
            \foreach \x in {10.6,11.6,12.6} {\draw[line width=0.8pt] (\x,0.4) -- (\x,4.4);}
            \foreach \y in {1.4,2.4,3.4} {\draw[line width=0.8pt] (9.6,\y) -- (13.6,\y);}
            \fill[gray!20] (9.6,0.4) rectangle (10.6,1.4);
            \fill[gray!20] (10.6,0.4) rectangle (11.6,1.4);
            \fill[gray!20] (11.6,0.4) rectangle (12.6,1.4);
            \fill[gray!20] (12.6,0.4) rectangle (13.6,1.4);
            \fill[gray!20] (12.6,1.4) rectangle (13.6,2.4);
            \fill[gray!20] (12.6,2.4) rectangle (13.6,3.4);
            \fill[gray!20] (12.6,3.4) rectangle (13.6,4.4);
            \draw[line width=1.1pt] (9.6,1.4) rectangle (12.6,4.4);
            \fill[gray!20] (9.9,-0.25) rectangle (10.35,0.2);
            \draw[line width=0.7pt] (9.9,-0.25) rectangle (10.35,0.2);
            \node[font=\scriptsize, anchor=west] at (9.8,-0.03) {shaded cells = fixed padding};

            % arrow 3
            \draw[-{Latex[length=3.2mm,width=2.0mm]}, line width=1.0pt] (14.0,2.0) -- (15.0,2.0);
            \node[font=\scriptsize, align=center, text width=1.6cm] at (14.5,3.15) {Hilbert\\order};

            % code boxes
            \node[font=\bfseries, align=center, text width=4.2cm] at (17.1,4.2) {Hilbert traversal $\rightarrow$ 16-bit code};
            \draw[rounded corners=2pt, line width=0.9pt] (15.4,2.6) rectangle (18.8,3.35);
            \node[align=center] at (17.1,2.98) {bits: $b_{15}\,\dots\,b_0$};
            \draw[rounded corners=2pt, line width=0.9pt] (15.4,1.35) rectangle (18.8,2.1);
            \node[align=center] at (17.1,1.73) {hex: \texttt{0x3A7F} (example)};
            \node[font=\scriptsize, align=left, text width=4.3cm] at (17.1,0.25) {Compact, locality-preserving representation for fast matching and grouping.};
        \end{tikzpicture}%
        }
        \caption{Construction of an interaction context signature. For each frame, the local traffic scene around the cutter is mapped to a 3$\times$3 semantic occupancy grid, mirror-normalised, embedded into a fixed 4$\times$4 grid, and encoded as a reversible 16-bit Hilbert-order code. The shaded cells are fixed padding and are not part of the original 3$\times$3 semantic grid.}
...
% ==============================================================================
  ...
  \item \textbf{Reconstruction path:} uses only trajectories and lane markings to produce the same outputs without using highD's derived relational fields as inputs.
    ...
    This two-path design enables like-for-like agreement measurements at the frame, event, and representation levels. Figure~\ref{fig:framework-overview} summarises the processing flow and highlights the evaluation boundary.
    ...
    \begin{figure}[p]
        ...
        \node[inputblock] (lanemeta) at (4.4, 0)
        ...
        \node[group,
              fit=(eval),
              label={[grouplabel]above:Evaluation boundary}
             ] {};

        \end{tikzpicture}
        \caption{Overview of \FrameworkName{}. The minimal-input reconstruction path uses trajectories, vehicle geometry, and lane markings to infer lanes, neighbour relations, cut-in events, pair-specific indicators, and context signatures. In parallel, a reference-label path derived from highD lane and neighbour identifiers produces outputs for agreement evaluation only.}
...
\ForEach{frame $t$ in the analysis window around $t_0$}{
    ...
    All results reported in this thesis are deterministic given the configuration in Table~\ref{tab:config} and the \textit{highD} dataset. The complete pipeline, including scripts for all tables and figures, is publicly available at \ArtifactURL.
...
\bottomrule
    ...
    \begin{figure}[H]
        ...
        \caption{Decision-stage occupancy maps. Top: absolute mean occupancy for non-risk and risky events; these panels are intentionally similar and are included mainly as context. Bottom: risk-minus-non-risk difference map, which is the main analytical view because it highlights the cells where occupancy frequency shifts between the two groups.}
    ...
    \begin{figure}[H]
        ...
        \safeincludegraphics[width=0.58\textwidth]{intention_diff.png}\\[6pt]
        \textbf{Decision stage}\\[2pt]
        \safeincludegraphics[width=0.58\textwidth]{decision_diff.png}\\[6pt]
        \textbf{Execution stage}\\[2pt]
        \safeincludegraphics[width=0.58\textwidth]{execution_diff.png}
        \caption{Stage-wise risk-minus-non-risk occupancy differences for the intention, decision, and execution stages, shown separately to make the small cell-wise shifts easier to read. Red indicates cells occupied more often in risky events, blue indicates the opposite, and cell labels report signed percentage-point differences. The centre cell is neutral because the cutter occupies it by construction.}
    ...
    \begin{figure}[H]
        \centering
        \safeincludegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{archetype_heatmaps.png}
        \caption{Seven decision-stage context archetypes derived from mirrored binary SFC signatures. After mirror normalisation, the right column corresponds to the target lane, so each panel visualises one canonical cut-in scene family. Cell labels show mean occupancy percentages, while the grey hatched centre cell is neutralised because the cutter occupies it by construction. The figure is intended to emphasise recurring occupancy templates rather than precise continuous gradients; the quantitative risk comparison for these archetypes is given in Figure~\ref{fig:sfc-archetype-risk}.}
    ...
    Kinematic features alone achieve slightly higher AUC (0.829) than the full model (0.824), while SFC features alone still achieve AUC 0.613, well above chance (0.50). For statistical interpretation we report two planned fold-level Wilcoxon signed-rank comparisons: SFC-only AUC versus chance level 0.50, and full-model AUC versus kinematic-only AUC. The corresponding uncorrected $p$-values are $1.65 \times 10^{-11}$ and $0.125$. Under a simple Bonferroni correction for these two comparisons, the adjusted values become $3.30 \times 10^{-11}$ and $0.25$, so the interpretation is unchanged: SFC-only performance remains clearly above chance, while the full model is not significantly better than the kinematic-only model. A broader omnibus Friedman test over a larger family of model variants is left for future work. The spatial context signature therefore carries meaningful information about future risk even without scalar speed or gap information, but the dominant predictive signal remains kinematic.
  ...
  \item \textbf{Reference agreement:} for cut-in stage frames, we compute a reference occupancy grid from highD surrounding-vehicle ids and compare its SFC code to the minimal-input code computed from inferred lanes and geometry. Exact matches indicate identical neighbour occupancy under the two definitions; non-matches localise differences to specific cells via Hamming distance.
    ...
    \section{Exploratory exiD extension}
    \label{sec:exid-exploratory}
    To show a small but concrete indication that the workflow can extend beyond \textit{highD}, we added a deliberately narrow exiD sidecar run~\cite{exid_2022}. The purpose was not to reproduce the four-level highD benchmark. Instead, the question was whether the explicit lane-change and cut-in mining path could be adapted to a second highway drone dataset with limited additional code and without changing the main thesis pipeline.

    The exiD extension therefore stays intentionally small. We process only 10 recordings, chosen to cover all 7 exiD locations while keeping runtime low. The extension reuses the same cut-in event definition and explicit cutter--follower pairing logic, but it does \emph{not} attempt full highD-style lane-marking reconstruction or context-signature validation. exiD differs structurally from highD: it provides lanelet-oriented identifiers and native leader-based headway measures, but not the same lane-marking metadata and surrounding-vehicle reference labels needed for the full highD validation chain. For that reason, the exiD add-on is used only for exploratory event mining and compact pairwise risk summaries.

    \begin{table}[H]
        \centering
        \caption{Compact summary of the exploratory exiD extension. The figures are reported only to illustrate limited transferability beyond highD, not as a second benchmark.}
        \label{tab:exid-exploratory}
        {\small
        \setlength{\tabcolsep}{4pt}
            \begin{tabular}{@{}rrrrrrr@{}}
\toprule
Recordings & Locations & Lane changes & Cut-ins & Exported events & Med. THW & Med. DHW \\
\midrule
10 & 7 & 722 & 424 & 424 & 2.01 s & 47.72 m \\
\bottomrule
            \end{tabular}}
    \end{table}
    ...
    \begin{figure}[H]
        \centering
        \safeincludegraphics[width=0.82\textwidth]{exid_exploratory_overview.png}
        \caption{Per-recording summary of the exploratory exiD subset. The left panel shows how many explicit cut-ins were exported from each selected recording, while the right panel shows the median execution-stage THW within that recording. The figure is intended to document subset-level spread, not to serve as a benchmark comparison against highD.}
        \label{fig:exid-exploratory-overview}
    \end{figure}

    Table~\ref{tab:exid-exploratory} shows that the selected subset still yields a non-trivial event set: 722 marked lane changes and 424 explicit cutter--follower cut-ins. The median execution-stage THW is \SI{2.01}{\second}, the median execution-stage DHW is \SI{47.72}{\meter}, and 46.5\% of exported events have finite positive TTC during the relation window. These numbers are not directly comparable to the highD benchmark because the exiD sidecar uses a reduced validation surface and dataset-native leader headway fields. Their role is only to demonstrate that the event-mining and pairwise risk-summary path can be transferred with limited dataset-specific adaptation.

    We also inspected two low-THW examples manually in the adapted viewer (recordings 42 and 70), shown in Figure~\ref{fig:exid-qualitative}. This qualitative check matters because the exiD extension is meant as a flavor of generalisation, not as a large quantitative expansion.

    \begin{figure}[H]
        \centering
        \safeincludegraphics[width=\textwidth]{exid_example_panels.png}
        \caption{Two qualitative exiD examples from the exploratory subset. Orange marks the cutter and blue marks the follower at the cut-in relation start frame. These panels are included only to illustrate that the adapted viewer and explicit pair construction remain usable on exiD; they are not presented as evidence equivalent to the main highD evaluation.}
        \label{fig:exid-qualitative}
    \end{figure}

    Taken together, the exiD results support the narrower claim that the framework's event-construction logic is not tied only to highD, while leaving the main evidence base and all benchmark conclusions anchored in highD.
...
% ==============================================================================
    ...
    From a software-engineering perspective, the work contributes a deterministic data-processing and validation pipeline rather than only a traffic-analysis result. The central engineering properties are explicit input contracts, reconstructable intermediate artefacts, reference-based testing, and reproducible outputs. These properties matter because downstream models, dashboards, and safety assessments are only as reliable as the scenario extraction logic that feeds them.
    ...
    \paragraph{Transfer beyond highD.} The exploratory exiD extension in Section~\ref{sec:exid-exploratory} makes the transfer question more concrete without expanding the thesis into a second benchmark. Processing only 10 recordings already yields 424 explicit cut-ins across 7 sites, which shows that the event definition and cutter--follower pairing logic are not tied only to highD. At the same time, the exiD exercise also shows that transfer is not free: lanelet-oriented semantics, different neighbour conventions, and the lack of highD-style lane-marking and surrounding-context reference labels immediately constrain which parts of the pipeline can be reused without additional validation work. The framework therefore reduces cross-dataset transfer to a bounded engineering task, but it does not eliminate dataset differences.

    \paragraph{Per-recording variability.}
    The cross-recording perspective also reveals notable variability. Risk prevalence ranges from 11.1\% to 34.1\% across the 60 recordings (median 20.0\%), which likely reflects differences in traffic density, speed distributions, and lane configurations across the six recording sites. LORO-CV fold-level AUC ranges from approximately 0.65 to 0.95, with lower-performing folds concentrated in recordings with fewer events or atypical speed distributions. This variability underscores the importance of the leave-one-recording-out protocol: pooled cross-validation would overestimate performance by allowing the model to learn site-specific patterns. It also suggests that broader cross-dataset evaluation would provide a stronger test of generalisability than within-highD analysis alone; the present exiD sidecar is only a first limited step in that direction.
...
\bottomrule
    ...
    \textbf{External validity:} The main quantitative results are based on \textit{highD} highway recordings collected at six German motorway sites under free-flow to moderately congested conditions. The framework's assumptions (straight road segments, clearly marked lanes, and moderate traffic density) may not hold for urban driving, ramp merges, construction zones, or different sensing setups. The small exiD extension shows that the event-mining core can be adapted beyond highD, but it does not establish full cross-dataset validity because the surrounding-context branch and the reference-based reconstruction evaluation remain tied to highD-specific metadata. NGSIM~\cite{ngsim_2016} was considered as an additional transfer dataset, but it was not included in the evaluated experiments because its export conventions, identity handling, and kinematic noise~\cite{thiemann2008ngsim} would require a separate harmonisation study.

    \textbf{Conclusion validity:} The thesis reports descriptive differences, sensitivity analyses, and cross-validated predictions rather than causal claims about driver behaviour. The fold-level Wilcoxon comparisons reported in the ablation study, together with the Bonferroni-adjusted interpretation, support the claim that the SFC-only model exceeds chance. However, the 60-fold LORO-CV still produces correlated folds because training sets overlap heavily, so these $p$-values should be read as supportive rather than definitive. A fuller repeated-measures comparison across a broader set of model variants would strengthen the statistical argument further.

    \section{Future work}
    Five directions for future work emerge from this thesis:

    \begin{enumerate}[leftmargin=1.6em]
  \item \textbf{Full cross-dataset validation:} extending the current exploratory exiD sidecar into a calibrated multi-dataset study, including coordinate harmonisation, kinematic smoothing, and representation-level validation beyond event mining. A future revisit of NGSIM would first require dedicated cleaning and neighbour-semantics calibration before it could be evaluated on comparable terms.
...
% ==============================================================================
    ...
    This is also where the software-engineering relevance lies. The framework defines explicit input boundaries, reconstructable intermediate products, and deterministic outputs that can be checked against reference labels. As a result, it is relevant not only for analysing highD but also as a basis for portable tooling for scenario mining, safety analysis, and validation workflows in automated-driving software engineering.

    A small exploratory exiD extension further supports this claim: on 10 recordings spanning 7 sites, the same lane-change and cut-in mining logic still yields 424 explicit cutter--follower events without altering the main highD benchmark pipeline. However, the exercise also makes the limitation clear. Cross-dataset transfer is constrained less by the event definition itself than by dataset metadata, neighbour semantics, and coordinate conventions. A full multi-dataset validation therefore remains future work, even if limited transferability is now illustrated with one small secondary dataset.
...
% ==============================================================================
    ...
    \begin{thebibliography}{99}
        ...
        \bibitem{exid_2022}
        J.~Moers, K.~Rudolph, H.~Niesen, S.~Barton, and L.~Eckstein.
        \newblock The exiD dataset: A real-world trajectory dataset of highly interactive highway scenarios in Germany.
        \newblock In \emph{2022 IEEE Intelligent Vehicles Symposium (IV)}, 2022.
        \newblock DOI: \url{https://doi.org/10.1109/IV51971.2022.9827305}.

        \bibitem{ngsim_2016}
        U.S. Department of Transportation Federal Highway Administration.
        \newblock Next Generation Simulation (NGSIM) vehicle trajectories and supporting data.
        \newblock Dataset, 2016.
        \newblock DOI: \url{https://doi.org/10.21949/1504477}.

        \bibitem{thiemann2008ngsim}
        C.~Thiemann, M.~Treiber, and A.~Kesting.
        \newblock Estimating acceleration and lane-changing dynamics based on NGSIM trajectory data.
        \newblock \emph{Transportation Research Record}, 2088(1):90--101, 2008.
        \newblock DOI: \url{https://doi.org/10.3141/2088-10}.
...
\bottomrule
  ...
  \item TTC is DHW divided by closing speed when the follower is gaining on the cutter; otherwise TTC is treated as infinite.
    ...
    \chapter{Appendix: Supplementary SFC interpretation views}
    \label{app:occupancy-supp}

    Figure~\ref{fig:sfc-decision-grids} in Chapter~\ref{chap:results} presents the decision-stage occupancy maps in the main text because that stage is the most relevant pre-onset comparison, while Figure~\ref{fig:sfc-stage-diff} provides the core signed difference view. This appendix adds two interpretation-first summaries that make the subtle stage-wise shifts easier to read than repeated absolute heatmaps. Figure~\ref{fig:sfc-stage-trends} follows each semantic cell across the intention, decision, and execution stages, and Figure~\ref{fig:sfc-stage-ranked} ranks the signed percentage-point shifts within each stage.

    \begin{figure}[H]
        \centering
        \safeincludegraphics[width=\textwidth]{stage_diff_trends.png}
        \caption{Stage-wise trajectories of the signed occupancy differences for each informative semantic cell. Positive values indicate higher occupancy in risky events, negative values indicate higher occupancy in non-risk events. This view makes it easier to see how the same local region changes from intention to decision to execution.}
        \label{fig:sfc-stage-trends}
    \end{figure}

    \begin{figure}[H]
        \centering
        \safeincludegraphics[width=\textwidth]{stage_diff_ranked.png}
        \caption{Ranked semantic-cell shifts within each stage, shown as signed percentage-point differences. This view makes it explicit which cells drive the contrast at each stage and confirms that the dominant pre-onset changes occur around the cells ahead of the cutter and, later, behind the manoeuvre.}
        \label{fig:sfc-stage-ranked}
    \end{figure}
