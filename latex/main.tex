...
% ==============================================================================
  ...
  \item rows: \{ahead of the cutter, alongside the cutter, behind the cutter\}.
    ...
    \begin{figure}[H]
        ...
        \end{tikzpicture}
        \caption{Construction of an interaction context signature. For each frame, the local traffic scene around the cutter is mapped to a 3$\times$3 semantic occupancy grid, mirror-normalised to a canonical direction, embedded into a fixed 4$\times$4 grid, and encoded as a reversible 16-bit Hilbert-order code. The shaded cells in the 4$\times$4 diagram are fixed padding and are not part of the original 3$\times$3 semantic grid.}
...
% ==============================================================================
  ...
  \item \textbf{Reconstruction path:} uses only trajectories and lane markings to produce the same outputs without using highD's derived relational fields as inputs.
    ...
    \begin{figure}[H]
        ...
        \end{tikzpicture}
        \caption{Overview of \FrameworkName{}. The minimal-input reconstruction path uses trajectories, vehicle geometry, and lane markings to infer lanes, neighbour relations, cut-in events, pair-specific indicators, and context signatures. In parallel, a reference-label path derived from highD lane and neighbour identifiers provides reference outputs for agreement evaluation only.}
